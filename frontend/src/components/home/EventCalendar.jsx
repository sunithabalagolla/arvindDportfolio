...
import { ChevronLeft, ChevronRight, Calendar, Clock, MapPin, Bell, CheckCircle, AlertCircle } from 'lucide-react';
...
export default function EventCalendar() {
    ...
    // Dynamic months based on available events
            ...
            try {
                ...
                if (response.success) {
                    const transformedEvents = response.data.map(event => ({
                        ...
                        image: event.image,
                        color: event.color,
                        status: event.status || 'published' // Backend now includes status
                    }));
                    ...
                    const uniqueMonths = ['All', ...new Set(
                        transformedEvents.map(event => event.month)
                    )];
                    setAvailableMonths(uniqueMonths);
                    
                    console.log(`✅ Loaded ${transformedEvents.length} upcoming events`);
                } else {
            ...
            if (isAuthenticated) {
                try {
                    ...
                    if (response.success) {
                        ...
                        console.log(`🔔 ${eventIds.size} notification(s) found`);
                    }
                } catch (error) {
            ...
            <div className="bg-white py-8 sm:py-12 px-4 sm:px-6">
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="text-center py-12">
                        ...
                        <p className="text-lg sm:text-xl text-gray-600">Loading upcoming events...</p>
                    </div>
                </div>
            </div>
        );
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="text-center mb-8 sm:mb-12 relative">
                    ...
                    <p className="text-sm sm:text-base text-gray-600 max-w-3xl mx-auto px-4 sm:px-0 mt-4">
                        {events.length > 0 
                            ? `Discover ${events.length} upcoming event${events.length !== 1 ? 's' : ''} - workshops, gatherings, and more!`
                            : 'Stay updated with all upcoming events, workshops, and gatherings.'
                        }
                    </p>
                </div>
                ...
                {filteredEvents.length > 0 ? (
                    ...
                    <div className="text-center py-12 bg-gray-50 rounded-2xl">
                        ...
                        <p className="text-lg sm:text-xl text-gray-600 px-4 mb-2">
                            {selectedMonth === 'All' 
                        ...
                        <p className="text-sm text-gray-500 px-4">
                            ...
                            {selectedMonth === 'All' && 'Check back soon for new events!'}
                        </p>
                    </div>
                )}
            </div>
...
// Added spinner animation while loading
// Error state with retry button
// Shows friendly messages

// 2. Dynamic Month Filter 📅

// Only shows months that have events
// Auto-populates from actual event data

// 3. Smart Empty States 🎯

// Different messages for "no events" vs "no events this month"
// Quick link to view all months

// 4. Improved UX ✨

// Event count in subtitle
// Disabled navigation buttons when at start/end
// Better console logging for debugging

// 5. Backend Integration 🔌

// Now works with filtered backend (only shows published/upcoming)
// Handles status field from backend
